                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-group list-group-responsive theme-details-list margin-bottom-3">
                            ...
                            @if($theme->link != null)
                                <li class="list-group-item"><strong>{{ trans('themes.showLink') }}</strong> <span> <a
                                                href="{{$theme->link}}" target="_blank" data-toggle="tooltip"
                                                title="Go to Link">{{$theme->link}}</a></span></li>
                            @endif

                            @if($theme->notes != null)
                                <li class="list-group-item"><strong>{{ trans('themes.showNotes') }}</strong>
                                    <span>{{ $theme->notes }}</span></li>
                            @endif

                            <li class="list-group-item"><strong>{{ trans('themes.showAdded') }}</strong>
                                <span>{{ $theme->created_at }}</span></li>
                            <li class="list-group-item"><strong>{{ trans('themes.showUpdated') }}</strong>
                                <span>{{ $theme->updated_at }}</span></li>
                        </ul>
                        ...
                        @if(count($themeUsers) > 0)
                            ...
                            <ul class="list-group">
                                @foreach ($themeUsers as $themeUser)
                                    <li class="list-group-item"><i class="fa fa-user fa-fw margin-right-1"
                                                                   aria-hidden="true"></i> {{ $themeUser->name }}</li>
                                @endforeach
                            </ul>
                        @endif
                        ...
                        <div class="row pt-2">
                            <div class="col-sm-6 mb-2">
                                <a href="/themes/{{$theme->id}}/edit" class="btn btn-small btn-info btn-block">
                                    <i class="fa fa-pencil fa-fw" aria-hidden="true"></i> Edit<span class="hidden-sm"> this</span><span
                                            class="hidden-sm"> Theme</span>
                                </a>
                            </div>
